...
function NewInvoice(props) {
  ...
  console.log(
    "uid in invoiceAct",
    useSelector((state) => state.firebase.auth.uid)
  );
  ...
  let urlIdStr = window.location.pathname;
  var urlId = urlIdStr.slice(9);
  console.log("window", urlId);
  ...
  const handleInvoiceSubmit = (metaData) => {
    const handleDataSubmit = (data) => {
      ...
      dispatch(createInvoice(finalObj, urlId));
    };
